...
export const Aurora: React.FC<AuroraProps> = (props) => {
  ...
  useEffect(() => {
    ...
    function resize() {
      if (!ctn) return;
      // Downscale resolution to 50% for high performance and smooth scrolling.
      // Since it's a smooth gradient aurora effect, rendering at lower res
      // and letting the browser upscale it gives it a softer look and saves massive GPU fillrate.
      const dpr = 0.5;
    ...
    const update = (t: number) => {
      ...
      const time = currentProps.time ?? (t * 0.01);
