...
export const OptionWheel: React.FC<OptionWheelProps> = ({
  ...
  // Fallback for window width / root font sizing
  ...
  // Synchronize when the active section index updates from the parent page
  useEffect(() => {
  ...
  // Wheel scrolling (registered manually for non-passive support)
  useEffect(() => {
  ...
  return (
    ...
    >
      {items.map((label, index) => (
        ...
        >
          <span className="option-wheel__item-label">
            <span className="option-wheel__item-dot">◈</span>
            {label}
          </span>
        </div>
      ))}
